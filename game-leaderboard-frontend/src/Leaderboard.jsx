...
const Leaderboard = () => {
    ...
    const fetchPlayers = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3005/api/leaderboard/6718bcd1fca6ec0d8cff67a3"
        );
        const playersData = response.data.leaderboard.map((player) => ({
          country: player.country,
          username: player.name,
          rank: player.rank || "N/A",
          money: player.earnings || 0,
  ...
  const columnDefs = [
    { headerName: "Rank", field: "rank", flex: 1, minWidth: 40 },
